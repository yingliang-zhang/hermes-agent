...
13. OMP effective route evidence is read from the attributed JSONL `model_change` and `thinking_level_change` records. Requested flags alone are insufficient. JSONL record order is not fixed, exactly one session record is required, and both wrapper/session cwd values are physically canonicalized before comparison so logical aliases such as macOS `/tmp` and `/private/tmp` cannot drift apart. Missing, ambiguous, malformed, or mismatched evidence fails closed and is recorded.
