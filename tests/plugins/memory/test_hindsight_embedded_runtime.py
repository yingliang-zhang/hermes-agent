...
def _clear_resolve_cache() -> None:
    """Clear the process-local seal cache so tamper tests re-run the full seal."""
    embedded_runtime._RESOLVE_CACHE.clear()
...
def test_seal_cache_hits_without_reseal_but_misses_on_fingerprint_change(tmp_path):
    """Cache: same (candidate, fingerprint) reuses the seal; a retargeted
    ``current`` symlink changes the fingerprint and forces a full re-seal."""
    server = _managed_server(tmp_path)
    assert resolve_server_executable(server.stable) == server.stable
    assert (server.stable, "a" * 64) not in embedded_runtime._RESOLVE_CACHE  # key is (candidate, fingerprint)

    # Same path, same fingerprint → cache hit, no tamper re-check (fast path).
    first_result = resolve_server_executable(server.stable)
    fingerprint = server.current.readlink().parts[1]  # e.g. "aaaa...aaaa" (64 hex)
    key = (str(server.stable), fingerprint)
    assert key in embedded_runtime._RESOLVE_CACHE
    # Second call must return the cached path (same resolved value).
    assert resolve_server_executable(server.stable) == first_result

    # Tampering a file inside the env does NOT re-trigger the full seal on a
    # cache hit (same fingerprint) — that is the accepted caching tradeoff.
    server.package_file.write_bytes(b"SERVER_VERSION = 2\n")
    assert resolve_server_executable(server.stable) == first_result

    # Retargeting `current` flips the fingerprint → cache miss → full re-seal
    # against the new (missing) environment → fail-closed RuntimeError.
    new_target = "b" * 64
    server.current.unlink()
    server.current.symlink_to(Path("envs") / new_target, target_is_directory=True)
    _clear_resolve_cache()  # ensure deterministic; fingerprint flip alone would miss
    with pytest.raises(RuntimeError):
        resolve_server_executable(server.stable)
...
def test_package_symlink_retarget_is_rejected(tmp_path):
    ...
    # NOTE: the retargeted symlink lies INSIDE the sealed environment, so the
    # (candidate, fingerprint) cache key is unchanged — a plain second call
    # would hit the cache. Clearing the cache forces the full tree re-seal,
    # which re-hashes the tree and detects the retargeted symlink.
    _clear_resolve_cache()
    with pytest.raises(RuntimeError, match="tree seal"):
        resolve_server_executable(server.stable)
