...
def _append_model_switch_marker(session: dict | None, *, model: str, provider: str) -> None:
    ...
    entry["display_kind"] = "model_switch"
...
def _session_info(
    ...
    *,
    ...
    # Epoch seconds the current turn started, or None when idle. The snapshot
    # keeps elapsed-time restoration coherent with origin/revision fencing.
...
def _drain_queued_prompt(rid, sid: str, session: dict) -> bool:
    ...
    try:
        if _session_uses_compute_host(session):
            ...
            _run_prompt_submit(rid, sid, session, queued["text"], origin="user", **run_kwargs)
    except Exception as exc:
        print(
...
def _live_session_payload(
    ...
    *,
    ...
    with session["history_lock"]:
        ...
        turn_started_at = turn_snapshot.get("turn_started_at")
    ...
    def run():
        ...
        try:
            ...
            try:
                ...
                if (
                    persist_user_message is not None
                    and ("persist_user_message" in run_parameters or accepts_kwargs)
                ):
