...
def close_interrupted_tool_sequence(messages: list, final_response: Any = None) -> bool:
    """Append a synthetic assistant turn when an aborted tail is a tool result.

    Some partial/error exits return before ``finalize_turn`` after a tool result
    has already been added to the transcript. Persisting that tail means the
    next user message lands as ``… tool → user`` — a role-alternation violation
    that strict providers reject. These non-interrupt exits also carry a real
    response explaining the failure, so closing the durable transcript with
    that response preserves both the delivered text and provider alternation.

    User-requested interrupts deliberately do not call this helper: interrupt
    status is metadata, and their canonical transcript stays free of synthetic
    assistant prose. The per-request API sanitizer closes those tails only on
    the wire copy when a later user message is sent.
