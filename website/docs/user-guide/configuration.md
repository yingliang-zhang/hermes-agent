...
compression:
  ...
  hygiene_hard_message_limit: 5000                  # All-surface count safety valve — see below
...
`hygiene_hard_message_limit` is an all-surface **pre-compression safety valve** used by gateway hygiene and the interactive CLI/TUI preflight and in-turn pre-API pressure checks. It breaks the death spiral where oversized requests disconnect before returning token usage, leaving token-based compression unable to fire. At or above this message count, Hermes forces bounded recovery even when ordinary automatic compression would be deferred by stale real-usage data, an active summary-failure cooldown, or repeated ineffective compactions. Preflight runs at most three passes and stops immediately when a pass makes no progress; in-turn recovery permits at most three attempts per turn before continuing to the provider. Below the count, all ordinary token thresholds and suppression gates keep their normal behavior. Default `5000` — far above normal sessions, including large-context (1M+) models that compress on token pressure first. Raise it for unusual high-turn workloads, lower it for an earlier backstop, or set it to `0` to disable the count valve. Editing this value on a running gateway takes effect on the next message (see below); CLI/TUI processes read it when the agent starts.
